...
import template from './../../templates/p-gameUI.ejs';

let deck = require ('./../3DDeck');
...
let GameUIView = Backbone.View.extend({
  ...
  initialize: function (myColor) {
    ...
    deck.init(this.$el).then( () => {
      ...
      Object.keys(sides).forEach((color) => {
        sides[color].on('piece_add', (piece) => {
          deck.addPiece(
            piece.attributes.type,
            piece.attributes.x,
            piece.attributes.y,
      ...
      sides['white'].addPiece(new Pieces.Knight ({x: 1, y: 0, color: 'white', enemyCollection: sides['black']}));
  ...
  //   };
    ...
    let onPlayerMove = (response) => {
      let oldX = StoN(response.from.x);
      let oldY = response.from.y - 1;
      let newX = StoN(response.to.x);
      let newY = response.to.y - 1;

      let movingPiece = sides[response.playerColor].getPieceAt(oldX, oldY);

      deck.movePiece({x: oldX, y: oldY}, {x: newX, y: newY});
      movingPiece.moveTo(newX, newY);
